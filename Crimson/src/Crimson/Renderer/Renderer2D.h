...
namespace Crimson {
	...
	public:
		...
		static void DrawQuad(const glm::vec3& pos, const glm::vec3& scale, Ref<Texture2D> tex, uint32_t index = 1, const float angle = 0.f);
		static void DrawQuad(const glm::mat4& transform, const glm::vec4& color, Ref<Texture2D> texture = nullptr);
		static void DrawSubTexturedQuad(const glm::vec3& pos, const glm::vec3& scale, Ref<SubTexture2D> tex, uint32_t index = 2, const float angle = 0.f);
