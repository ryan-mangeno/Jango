...
fragment float4 fragment_main(VertexOutput in [[stage_in]],
                              constant FragmentUniforms& fragUniforms [[buffer(1)]],
                              texturecube<float> environmentMap [[texture(0)]])
{
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        float3 H = ImportanceSampleGGX(Xi, N, fragUniforms.roughness);
        float3 L = normalize(2.0 * dot(V, H) * H - V);

        float NdotL = max(dot(N, L), 0.0);
        if(NdotL > 0.0)
        {
            float D = DistributionGGX(N, H, fragUniforms.roughness);
            float NdotH = max(dot(N, H), 0.0);
            ...
            float mipLevel = fragUniforms.roughness == 0.0 ? 0.0 : 0.5 * log2(saSample / saTexel); 
            
            prefilteredColor += environmentMap.sample(textureSampler, L, level(mipLevel)).rgb * NdotL;
            totalWeight += NdotL;
        }
    }
